...
static void __devinit geth_net_setup(struct net_device *dev){
    ether_setup(dev);
}
...
autoconf_fail:
	...
	net = alloc_netdev ( sizeof *dev , "gth" , geth_net_setup );
	
	dev_err (&gadget->dev,"net=%x priv=%X struct size=%d\n",net,&net->priv,sizeof *net);
    p  = (unsigned int * )net;
    for( loopi=0; loopi < (sizeof(struct net_device)+sizeof (struct eth_dev))/16; loopi++,p+=4){
		dev_err (&gadget->dev,"%03d %08X %08X %08X %08X <== %08X\n",loopi*4,*p,*(p+1),*(p+2),*(p+3),p);
    }
	...
	netif_carrier_off (dev->net);
//============================================================================
dev_err (&gadget->dev,"controller '%s'  %d\n",gadget->name , __LINE__);
