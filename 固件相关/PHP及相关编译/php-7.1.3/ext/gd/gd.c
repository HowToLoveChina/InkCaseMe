...
ZEND_BEGIN_ARG_INFO_EX(arginfo_imagefile, 0, 0, 1)
	ZEND_ARG_INFO(0, im)
	ZEND_ARG_INFO(0, to)
...
#endif
	PHP_FE(imagefile,								arginfo_imagefile)
#ifdef HAVE_GD_WEBP
...
/* }}} */
#endif /* HAVE_GD_PNG */
/* {{{ proto bool imagefile(resource im [, string to , int format])
   Output raw to file */
PHP_FUNCTION(imagefile)
{
	zval *IM;
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "rsl", &IM,&to,&to_len,&format) == FAILURE) {
		return;
	}
	if ((im = (gdImagePtr)zend_fetch_resource(Z_RES_P(IM), "Image", le_gd)) == NULL) {
		RETURN_FALSE;
	...
	if (width == 0 || height ==0) RETURN_FALSE;
